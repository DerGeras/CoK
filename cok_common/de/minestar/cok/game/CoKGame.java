...
public class CoKGame {
	...
	public static void startGame() {
		...
		for (Team team : teams.values()) {
			...
			if (team.getAllPlayers().size() > 0) {
				ChatSendHelper.broadCastMessage("Team " + Color.getColorCodeFromChar(team.getColor()) + team.getName() + Color.getColorCodeFromString("gray") + " with leader " + Color.getColorCodeFromString("white") + team.getCaptain());
			}
	...
	public static void stopGame() {
		...
		if (CoKGame.teams.size() > 0) {
			...
			for (Team team : teams.values()) {
				...
				ChatSendHelper.broadCastMessage(Color.getColorCodeFromChar(team.getColor()) + team.getName() + Color.getColorCodeFromString("white") + ": " + getScoreForTeam(team) + "/" + maxScore);
				// TODO finish call
			}
		}
		// send state to clients
	...
	/**
	 * sort unsorted sockets. Needed this due to divergation in the call
	 * "socket.getBlockMetadata()"
	 */
	...
	/**
	 * Sets a player to spectator mode Sets capabilities
	 * 
	 * @param player
	 *            EntityPlayer
	 */
	...
	/**
	 * Remove Spectator sets capabilities
	 * 
	 * @param playername
	 */
	...
	/**
	 * punish a team by adding blocks to the tower i.e. when the leader dies
	 * 
	 * @param team
	 * @param amount
	 */
