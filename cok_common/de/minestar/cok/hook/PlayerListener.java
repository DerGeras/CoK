...
public class PlayerListener {
	...
	public void onPlayerDeath(PlayerDropsEvent event) {
		...
		for (EntityItem itemEntity : event.drops) {
			if (!(CoKGame.playerProfessions.get(name) != null && CoKGame.playerProfessions.get(name).givenItems.contains(itemEntity.getEntityItem().getItem()))) {
		...
		if (teamCaptain.equals(name)) {
			...
			event.drops.add(new EntityItem(event.entityPlayer.worldObj, event.entityPlayer.posX, event.entityPlayer.posY, event.entityPlayer.posZ, headStack));
			// change captain
	...
	public void onPlayerTossItem(ItemTossEvent event) {
		if (!event.player.capabilities.isCreativeMode) {
			...
			if (CoKGame.playerProfessions.get(event.player.username) != null && CoKGame.playerProfessions.get(event.player.username).givenItems.contains(stack.getItem())) {
